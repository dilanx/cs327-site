  ...
  new Vue({
    template: `
      <div id="app">
        ...
        <div class="tools">
          <label>
            <input type="checkbox" v-model="settings.drawDebugInfo" />
            DEBUG DRAW
          </label>
          <div class="systems">
            <button v-for="system in displaySystems" 
              :class="{active: system.isActive}"
              class="button active-green"
          ...
          <div class="system-controls-container">
            <div v-for="system in activeSystems" class="system-controls">
              <div class="system-controls-title">
                ...
                <p class="name">{{system.displayName}}</p>
              </div>
              <component :is="'controls-' + system.name"  />
              <div v-if="system.tips" class="system-controls-tips">
                <p class="label">TIPS</p>
                <p class="tip">{{system.tips}}</p>
              </div>
          </div>
			  </div>
		  </div>
      ...
      new p5((p) => {
        ...
        p.mousePressed = () => {
          ...
          this.activeSystems.forEach((s) =>
            s.mousePressed?.(p, this.held, this.settings)
          );
        };
        p.mouseReleased = () => {
          // If any of the active systems have this event, trigger it
          this.activeSystems.forEach((s) =>
            s.mouseReleased?.(p, this.held, this.settings)
          );

          if (this.held) {
            let m = 20;
            this.held.velocity = new Vector2D(...p.mouseVelocity);
        ...
        p.mouseDragged = () => {
          ...
          // If any of the active systems have this event, trigger it
          this.activeSystems.forEach((s) =>
            s.mouseDragged?.(p, this.held, this.settings)
          );
        };
        ...
        p.mouseClicked = () => {
          // If any of the active systems have this event, trigger it
          this.activeSystems.forEach((s) =>
            s.mouseClicked?.(p, this.held, this.settings)
          );
        };
    ...
    methods: {
      toggleSystem(sys) {
        ...
        if (sys.isActive) {
          console.log('activate system: ', sys.name);
