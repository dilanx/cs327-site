class Vector2D {
  ...
  getClosest(pts, { getPosition, getRadius, range = 20 } = {}) {
    ...
    pts.forEach((pt) => {
      ...
      const distance =
        Math.sqrt((this.x - pos.x) ** 2 + (this.y - pos.y) ** 2) - radius;
  ...
  setTo(...args) {
    ...
    if (typeof args[0] == Object && !isNaN(args[0].x) && !isNaN(args[1].y)) {
      this.x = args[0];
      this.y = args[1];
      return this;
    }
  ...
  setToEdgePoint({ pt0, pt1, v, pct = 0, edgeOffset = 0, normalOffset = 0 }) {
    ...
    if (!v) v = new Vector2D(pt1.x - pt0.x, pt1.y - pt0.y);
  ...
  addMultiple(...args) {
    ...
    if (args.length % 2 !== 0) {
      throw new Error(
        'Expecting an even number of arguments. Pairs of Vector2D (or similar) and scalars are required.'
      );
    }
    ...
    for (let i = 0; i < args.length / 2; i++) {
      ...
      if (typeof v.x !== 'number' || typeof v.y !== 'number') {
        console.warn(v);
        throw new Error(
          `Expecting an object with x and y properties at index ${i * 2}.`
        );
      }
      ...
      if (typeof m !== 'number') {
        throw new Error(
          `Expecting a number at index ${i * 2 + 1}. Got ${typeof m}`
        );
      }
  ...
  drawArrow(
    ...
    let end = Vector2D.edgePoint({
      pt0: this,
      v,
      pct: multiplyLength,
      normalOffset,
      edgeOffset: endOffset,
    });
