...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  new Vue({
      ...
      <div id="app">
        ...
        <div class="system-tools">
          ...
          <div class="system-tools-item">
            <button
              v-for="mode in modes"
              @click="animationMode=mode"
              class="button"
              :class="{active: animationMode === mode}"
            >
              {{mode}}
            </button>
          </div>
          ...
          <div class="system-tools-item">
            <div class="labeled-slider">
              <label>MUTATION RATE: {{mutationRate}}</label>
              <input
                type="range"
            ...
            <div class="labeled-slider">
              <label>POPULATION: {{populationCount}}</label>
              <input
                type="range"
                v-model.number="populationCount"
                min="1"
                max="20"
              />
            </div>
        ...
        <div class="individual-tools">
          <div class="col left">
            <p>DNA for Individual {{selectedIndividualIndex}}</p>

            <div class="landmarks">
              <p>SET TO LANDMARK</p>
              <div class="landmarks-buttons">
                <button 
          ...
          <div class="col right">
            <table>
              <tr v-for="dimName, index in selectedSpace.dimensions">
                <td class="slider-label">{{dimName}}</td>
                <td>
                  <input
                    type="range"
                    v-model.number="selectedIndividual.dna[index]"
                    min="0"
                    max="1"
                    step=".01"
                  />
                </td>
                <td>{{selectedIndividual.dna[index].toFixed(2)}}</td>
              </tr>
            </table>
          </div>
        </div>
      ...
      new p5((p) => {
        ...
        p.draw = () => {
          ...
          toDraw.forEach((individual) =>
            this.selectedSpace.updateIndividual?.(individual, this.settings)
          );
    ...
    methods: {
      startSpace() {
        ...
        this.population.forEach((individual) =>
          this.selectedSpace.setupIndividual?.(individual, this.settings)
        );
      },
      stopAnimating() {
      ...
      calculateBasePositions() {
        ...
        let cols = 3;
        if (this.populationCount == 1) cols = 1;
        if (this.populationCount > 4) cols = 5;
        ...
        for (var i = 0; i < this.populationCount; i++) {
          ...
          let border = 120 - this.populationCount;

          let x = cols == 1 ? 0.5 : (i % cols) / (cols - 1);

          let y = Math.floor(i / cols);
          x += (y % rows) * 0.1 - (rows - 1) * 0.04;
          let x2 = map(x, 0, 1, border, this.settings.p.width - border);
          let y2 = this.settings.p.height - 50 - 50 * y;

          ind.baseScale = 1.2 - (0.4 * y ** 0.4 + 0.01 * this.populationCount);
      ...
      copyToClipboard() {
        let dnaText = this.selectedIndividual.dna
          .slice(0, this.currentDNALength)
          .map((s) => s.toFixed(2));
    ...
    data() {
      ...
      let selectedSpace =
        latentSpaces.find((s) => s.name === lastSpace) ||
